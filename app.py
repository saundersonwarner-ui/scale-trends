...
# These files will be created locally on whoever is running the script
...
with st.sidebar:
    ...
    m1_label = st.text_input("Measurement 1", value=settings['M1_Name'])
    m2_label = st.text_input("Measurement 2", value=settings['M2_Name'])
    ...
    st.caption("Use these to move your data between devices.")
    
    uploaded_file = st.file_uploader("Import CSV", type="csv")
    if uploaded_file is not None:
        if st.button("Merge with Local Data", use_container_width=True):
            try:
    ...
    st.download_button("Export/Backup CSV", data=csv_out, file_name="scale_trends_backup.csv", mime='text/csv', use_container_width=True)

    st.divider()
    view_option = st.selectbox("View Range", ["Monthly", "Quarterly", "Yearly", "All Time"])

# --- INPUT SECTION ---
with st.expander("➕ Add or Edit Log", expanded=st.session_state.data.empty):
...
if not df.empty:
    ...
    with tab_w:
        ...
        if not valid_w.empty:
            ...
            # Prediction using Trimmed Trend
            if len(df) >= 10 and 'Weight_Trend' in df.columns:
                ...
                st.info("⏳ Need 10 days of data for prediction.")
    ...
    with tab_m:
        ...
        if not m_df.empty:
            ...
            fig_m.update_layout(margin=dict(l=10, r=10, t=30, b=10), height=350, legend=dict(orientation="h", y=1.1), xaxis=dict(fixedrange=True), yaxis=dict(fixedrange=True))
            st.plotly_chart(fig_m, use_container_width=True, config=chart_cfg)
else:
    st.info("👋 **Welcome to Scale Trends!** Use the box above to log your first weight. Your data stays on your device.")
